...
document.formDate.addEventListener('submit', function (e) {
  ...
  if (yearNeed !== 0 && yearNeed <= new Date().getFullYear()) {
    const minDate = new Date(yearNeed, 0, 1, 7);
    const maxDate = new Date(yearNeed, 11, 31, 7);
    ...
    const phieuThuePhongs = [
      ...
      return (
        dateDen.getFullYear() === yearNeed || dateDi.getFullYear() === yearNeed
      );
    });
    ...
    phieuThuePhongs
      .filter((phieu) => {
        const dateDi = new Date(phieu.querySelector('NGAYDI').innerHTML);
        return dateDi.getFullYear() === yearNeed;
      })
      .forEach((phieu) => {
